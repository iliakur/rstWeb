...
def structure_main(user, admin, mode, **kwargs):
    ...
    if mode == "server":
        ...
        header = header.replace(
            "**logout_control**", '(<a href="logout.py">log out</a>)'
        )
    else:
    ...
    help_match = re.search(
        r'(<div id="help_edit".*?</div>)', help, re.MULTILINE | re.DOTALL
    )
        ...
        if len(theform["action"]) > 1:
            ...
            if len(action_log) > 0:
                ...
                for action in actions:
                    ...
                    action_params = (
                        action.split(":")[1] if len(action.split(":")) > 1 else ""
                    )
                    params = action_params.split(",")
                    if action_type == "up":
                        update_parent(
                            params[0], params[1], current_doc, current_project, user
                        )
                    elif action_type == "sp":
                        insert_parent(
                            params[0],
                        ...
                        update_signals(
                            action.split(":")[1:], current_doc, current_project, user
                        )
                    else:
    ...
    for row in rows:
        ...
        if row[5] == "edu":
            nodes[row[0]] = NODE(
                row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], relkind
            )
        else:
            nodes[row[0]] = NODE(
                row[0], 0, 0, row[3], row[4], row[5], row[6], row[7], relkind
            )
    ...
    for signal in get_signals(current_doc, current_project, user):
        ...
        if tokens:
            ...
            tokens = []
        signals[s_id].append({"type": s_type, "subtype": subtype, "tokens": tokens})
    cpout += "<script>"
    cpout += "window.rstWebSignals = " + json.dumps(signals) + ";"
    cpout += (
        "window.rstWebSignalTypes = "
        + json.dumps(
            get_signal_types_dict(current_doc, current_project), sort_keys=True
        )
        + ";"
    )
    cpout += (
    ...
    for key in sorted(nodes, key=lambda id: nodes[id].depth, reverse=True):
        ...
        if node.parent != "0":
            ...
            if node.relname == "span":
                ...
                lr = get_multinuc_children_lr(
                    node.parent, current_doc, current_project, user
                )
                lr_wid = (lr[0] + lr[1]) / 2
                lr_ids = get_multinuc_children_lr_ids(
                    node.parent, lr[0], lr[1], current_doc, current_project, user
                )
                left_child = lr_ids[0]
                ...
                if left_child == right_child:
                    ...
                    if (
                        left_child in anchors and right_child in anchors
                    ):  # both leftmost and rightmost multinuc children have been found
                        len_left = nodes[left_child].right - nodes[left_child].left + 1
                        len_right = (
                            nodes[right_child].right - nodes[right_child].left + 1
                        )
                        anchors[parent.id] = str(
                            (
                                (
    ...
    for key in nodes:
        ...
        pix_anchors[node.id] = str(
            int(
                ...
                - 100
                - 39
                + float(anchors[node.id]) * ((node.right - node.left + 1) * 100 - 4)
            )
        )
    ...
    if int(get_schema()) > 2:
        ...
        use_multinuc_buttons = (
            True if get_setting("use_multinuc_buttons") == "True" else False
        )
    else:
    ...
    for key in sorted(nodes.keys(), key=int):
        ...
        if node.kind != "edu":
            ...
            cpout += (
                '<div id="lg'
                + node.id
                + '" class="group" style="left: '
                + str(int(node.left * 100 - 100))
                + "; width: "
                + g_wid
                + "; top:"
                + str(int(top_spacing + layer_spacing + node.depth * layer_spacing))
                + 'px; z-index:1"><div id="wsk'
                + node.id
            ...
            cpout += (
                '<div id="g'
                + node.id
                + '" class="num_cont" style="position: absolute; left:'
                + pix_anchors[node.id]
                + "px; top:"
                + str(int(4 + top_spacing + layer_spacing + node.depth * layer_spacing))
                + "px; z-index:"
                + str(int(200 - (node.right - node.left)))
                + '"><table class="btn_tb"><tr><td rowspan="2"><button id="unlink_'
                + node.id
                + '"  title="unlink this node" class="minibtn" onclick="act('
                + "'up:"
                + node.id
                + ",0'"
                + ');">X</button></td><td rowspan="2"><span class="num_id">'
                + str(int(node.left))
                + "-"
                + str(int(node.right))
                + "</span></td>"
            ...
            if use_span_buttons:
                cpout += (
                    '<td><button id="aspan_'
                    + node.id
                    + '" title="add span above" class="minibtn" onclick="act('
                    + "'sp:"
                    + node.id
                    + "'"
                    + ');">T</button></td>'
            ...
            if use_multinuc_buttons:
                cpout += (
                    '<tr><td><button id="amulti_'
                    + node.id
                    + '" title="add multinuc above" class="minibtn" onclick="act('
                    + "'mn:"
                    + node.id
                    + "'"
                    + ');">'
            ...
            cpout += (
                '<div id="wsk'
                + node.id
                + '" class="whisker" style="width:96px;"></div><div class="edu_num_cont"><table class="btn_tb"><tr><td rowspan="2"><button id="unlink_'
                + node.id
                + '" title="unlink this node" class="minibtn" onclick="act('
                + "'up:"
                + node.id
    ...
    for key in nodes:
        ...
        if node.kind == "edu":
            hidden_val += (
                ...
                + safe_relname
                + ","
                + get_rel_type(node.relname, current_doc, current_project)
                + ";"
            )
        elif node.kind == "span":
            hidden_val += (
    ...
    cpout += (
        '<input id="def_multi_rel" type="hidden" value="'
        + get_def_rel("multinuc", current_doc, current_project)
        + '"/>\n'
    )
    cpout += (
        '<input id="def_rst_rel" type="hidden" value="'
        + get_def_rel("rst", current_doc, current_project)
        + '"/>\n'
    )
    ...
    cpout += (
        ...
        + get_project_validations(current_project)
        + '"/>\n'
    )
    cpout += (
        '<input id="use_span_buttons" type="hidden" value="'
        + str(use_span_buttons)
        + '"/>\n'
    )
    cpout += (
        '<input id="use_multinuc_buttons" type="hidden" value="'
        + str(use_multinuc_buttons)
        + '"/>\n'
    )
    ...
    cpout += (
        'if (options =="multi"){options = multi_options;} else {options=rst_options;}'
    )
    cpout += (
        '        return options.replace("<option value='
        + "'"
        + '"'
        + "+my_rel+"
        + '"'
    ...
    cpout += "var signalsEnabled = " + (
        "true;" if get_setting("signals") == "True" else "false;"
    )
    ...
    for key in nodes:
        ...
        cpout += (
            'jsPlumb.makeSource("'
            + node_id_str
            + '", {anchor: "Top", filter: ".num_id", allowLoopback:false});'
        )
        cpout += (
            'jsPlumb.makeTarget("'
            + node_id_str
