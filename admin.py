...
def admin_main(user, admin, mode, **kwargs):
    ...
    if admin == "0":
        cpout += (
            '<p class="warn">User '
            + user
            + " does not have administrator rights!</p></body></html>"
        )
        return cpout
    else:
    ...
    help_match = re.search(
        r'(<div id="help_admin".*?</div>)', help, re.MULTILINE | re.DOTALL
    )
    ...
    if "guidelines_url" in theform:
        if "::" in theform["guidelines_url"]:
            ...
            if len(project_list) > 0:
                ...
                url_message = (
                    '<p class="warn">Added the URL '
                    + guideline_url
                    + " to the selected projects</p>"
                )
    ...
    if "edit_validation" in theform:
        if "::" in theform["edit_validation"]:
            ...
            validation_message = (
                '<p class="warn">Updated validation settings for project ' + val_project
            )
    ...
    cpout += (
        '''<br><br><label class="switch">
      <input type="checkbox" id="check_empty_span" onclick="admin('toggle_validations');"'''
        + checked
        + """>
   ...
   <span><img src="img/empty_span.png" height="100px">
    ...
    cpout += (
      ...
      <input type="checkbox" id="check_flat_rst" onclick="admin('toggle_validations');"'''
        + checked
        + """>
      <div class="slider round"></div>
...
"""
    ...
    cpout += (
        '''<br><label class="switch">
          <input type="checkbox" id="check_mononuc" onclick="admin('toggle_validations');"'''
        + checked
        + """>
          <div class="slider round"></div>
    ...
    if "file" in theform and "imp_project" in theform:
        ...
        if isinstance(fileitem, list):
            ...
            for filelist_item in fileitem:
                if (
                    filelist_item.filename and len(imp_project) > 0
            ...
            if (
                fileitem.filename and len(imp_project) > 0
            ):  # Test if the file was uploaded and a project selection exists
                #  strip leading path from file name to avoid directory traversal attacks
    ...
    cpout += (
        '</div><div class="tab_btn" id="docs_btn" onclick="open_tab('
        + "'"
        + "docs"
        + "'"
        + ');">Documents</div><div id="docs" class="tab">'
    )
        ...
        if len(doclist) > 0 and theform["delete"] == "delete":
            ...
            if len(docs_to_delete) > 0:
                if isinstance(docs_to_delete, list):
                    ...
                    delete_document(
                        docs_to_delete.split("/")[1], docs_to_delete.split("/")[0]
                    )
    ...
    if not docs:
        cpout += (
            "<p>No documents found with permissions for user name: " + user + "</p>"
        )
    else:
    ...
    if "export" in theform:
        ...
        if len(export_doc_list) > 0 and theform["export"] == "export":
            ...
            if isinstance(export_docs, list):
                ...
                export_document(
                    export_docs.split("/")[1], export_docs.split("/")[0], exportdir
                )
            ...
            if isinstance(users, list):
                for user in users:
                    try:
                        ...
                        user_del_message = (
                            '<p class="warn">Deleted users from selection</p>'
                        )
                    except OSError as e:
                        if e.errno == errno.ENOENT:
                            user_del_message = (
                                '<p class="warn">File to delete <i>'
                                + user
                                + "</i> not found!</p>"
                            )
                        else:
    ...
    if "assign_user" in theform:
        if len(theform["assign_user"]) > 0:
            ...
            users = (
                users_to_assign.split(";")
                if ";" in users_to_assign
                else [users_to_assign]
            )
            docs = (
                docs_to_assign.split(";") if ";" in docs_to_assign else [docs_to_assign]
            )
            for user in users:
                for doc in docs:
    ...
    if "new_user_data" in theform:
        if len(theform["new_user_data"]) > 0:
            ...
            if username in RESERVEDNAMES:
                ...
                if len(realname) < 1:
                    user_create_message = (
                        '<p class="warn">The real name cannot be empty!</p>'
                    )
                else:
                    if not is_email(email):
                        user_create_message = (
                            '<p class="warn">Invalid e-mail address: <b>'
                            + email
                            + "</b></p>"
                        )
                    else:
    ...
    if mode == "server":
        cpout += (
            '</div><div class="tab_btn" id="users_btn" onclick="open_tab('
            + "'"
            + "users"
            + "'"
            + ');">Users</div><div id="users" class="tab">'
        )
    else:
    ...
    for userfile in sorted(userfiles):
        if (
            userfile != "config.ini"
            and userfile != "default.ini"
            and userfile != "admin.ini"
            and userfile.endswith(".ini")
        ):
    ...
    if not docs:
        cpout += (
            "<p class='warn'>No documents found with permissions for user name: "
            + user
            + "</p>"
        )
    else:
    ...
    if len(assigned_users) > 0:
        ...
        cpout += (
            '<select id="assigned_doc_sel" name="assigned_doc_sel" class="doclist">'
        )
        for doc in assigned_docs:
            cpout += (
                '<option value="'
    ...
    ### database
    cpout += (
        '</div><div class="tab_btn" id="database_btn" onclick="open_tab('
        + "'"
        + "database"
        + "'"
        + ');">Database</div><div id="database" class="tab">'
    )
    ...
    cpout += (
        """<button onclick="admin('switch_signals')">Turn """
        + ("on" if opposite_signals == "True" else "off")
        + """</button>"""
    )
    ...
    for fname in signals_files:
        ...
        selected_string = (
            ' selected="selected"' if signals_file and signals_file == fname else ""
        )
        cpout += (
            '<option value="'
            + fname
    ...
    cpout += (
        """<button onclick="admin('switch_logging')">Turn """
        + opposite_logging
        + """</button>"""
    )
    ...
    cpout += (
        """<button onclick="admin('switch_span_buttons')">"""
        + opposite_span
        + """ span buttons</button><br/><br/>"""
    )
    cpout += (
        """<button onclick="admin('switch_multinuc_buttons')">"""
        + opposite_multinuc
        + """ multinuc buttons</button>"""
    )
