...
def setup_db():
    ...
    # Create tables
    cur.execute(
        """CREATE TABLE IF NOT EXISTS rst_nodes
    ...
    cur.execute(
        """CREATE TABLE IF NOT EXISTS rst_signals
...
def update_schema():
    ...
    if (
        schema < 4
    ):  # versions below 4 do not have last save timestamp to prevent resubmit on browser refresh
    ...
    for doc, project in get_all_docs_by_project():
        for majtype in types:
            for subtype in types[majtype]:
                generic_query(
                    "INSERT INTO rst_signal_types VALUES(?,?,?,?)",
...
def set_timestamp(user, timestamp):
    ...
    if schema >= 4:
        if timestamp != "" and user != "":
            ...
            generic_query(
                "INSERT or REPLACE INTO users ('user','timestamp') VALUES (?,?)",
                (user, timestamp),
            )
...
def get_project_validations(project):
    ...
    if schema < 5:
        update_schema()
    if len(project):
        validation_row = generic_query(
            "SELECT validations FROM projects WHERE project = ?", (project,)
        )
    else:
        return ""
...
def set_project_validations(project, validations):
    generic_query(
        "UPDATE projects SET validations=? WHERE project=?;", (validations, project)
    )
...
def import_document(filename, project, user, do_tokenize=False):
    ...
    for signal in rst_signals:
        cur.execute(
            "INSERT INTO rst_signals VALUES(?,?,?,?,?,?,?)",
            (signal[0], signal[1], signal[2], signal[3], doc, project, user),
        )  # user's instance
        cur.execute(
            "INSERT INTO rst_signals VALUES(?,?,?,?,?,?,?)",
            (signal[0], signal[1], signal[2], signal[3], doc, project, "_orig"),
        )  # backup instance
    ...
    for key in rel_hash:
        ...
        cur.execute(
            "INSERT INTO rst_relations VALUES(?,?,?,?)",
            (rel_name, rel_type, doc, project),
        )
...
def get_def_rel(relkind, doc, project):
    rel_row = generic_query(
        "SELECT relname FROM rst_relations WHERE reltype = ? and doc=? and project=? ORDER BY relname",
        (relkind, doc, project),
    )
    if len(rel_row) == 0:
        if relkind == "rst":
...
def get_docs_by_project(user):
    return generic_query(
        "SELECT doc, project, user FROM docs WHERE user=? ORDER BY project, doc COLLATE NOCASE",
        (user,),
    )
...
def get_all_docs_by_project():
    return generic_query(
        "SELECT DISTINCT doc, project FROM docs ORDER BY project, doc COLLATE NOCASE",
        (),
    )


def add_node(
    node_id, left, right, parent, rel_name, text, node_kind, doc, project, user
):
    generic_query(
        "INSERT INTO rst_nodes VALUES(?,?,?,?,?,?,?,?,?,?,?)",
        (
...
def create_project(project_name):
    generic_query(
        "INSERT OR IGNORE INTO projects (project) VALUES (?)", (project_name,)
    )
...
def update_parent(node_id, new_parent_id, doc, project, user):
    ...
    generic_query(
        "UPDATE rst_nodes SET parent=? WHERE id=? and doc=? and project=? and user=?",
        (new_parent_id, node_id, doc, project, user),
    )
    if new_parent_id == "0":
    ...
    if new_parent_id != "0":
        if get_kind(new_parent_id, doc, project, user) == "multinuc":
            ...
            get_rel(node_id, doc, project, user) == "span"
            and get_kind(new_parent_id, doc, project, user) != "span"
        ):  # A span child was just attached to a non-span
            update_rel(node_id, get_def_rel("rst", doc, project), doc, project, user)
    if prev_parent:
        if (
            not count_children(prev_parent, doc, project, user) > 0
            and not prev_parent == "0"
        ):  # Parent has no more children, delete it
...
def get_parent(node_id, doc, project, user):
    parent_row = generic_query(
        "SELECT parent FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
...
def get_rel(node_id, doc, project, user):
    rel_row = generic_query(
        "SELECT relname FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
        (node_id, doc, project, user),
    )
    return rel_row[0][0]
...
def get_kind(node_id, doc, project, user):
    if node_id == "0":
        ...
        rel_row = generic_query(
            "SELECT kind FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
            (node_id, doc, project, user),
        )
        return rel_row[0][0]
    ...
    if get_kind(parent_id, doc, project, user) == "multinuc":
        ...
        if new_rel_type == "rst":
            ...
            if (
                ...
                and get_rel_type(get_rel(node_id, doc, project, user), doc, project)
                == "multinuc"
            ):
            ...
            generic_query(
                "UPDATE rst_nodes SET relname=? WHERE id=? and doc=? and project=? and user=?",
                (new_rel, node_id, doc, project, user),
            )
            ...
            for child in children:
                if (
                    get_rel_type(get_rel(child[0], doc, project, user), doc, project)
                    == "multinuc"
                ):
                    generic_query(
                        "UPDATE rst_nodes SET relname=? WHERE id=? and doc=? and project=? and user=?",
                        (new_rel, child[0], doc, project, user),
                    )
    else:
        generic_query(
            "UPDATE rst_nodes SET relname=? WHERE id=? and doc=? and project=? and user=?",
...
def count_children(node_id, doc, project, user):
    count = generic_query(
        "SELECT count(*) FROM rst_nodes WHERE parent=? and doc=? and project=? and user=?",
        (node_id, doc, project, user),
    )
...
def count_multinuc_children(node_id, doc, project, user):
    count = generic_query(
        "SELECT count(rst_nodes.id) FROM rst_nodes JOIN rst_relations ON rst_nodes.relname = rst_relations.relname and rst_nodes.doc = rst_relations.doc and rst_nodes.project = rst_relations.project WHERE reltype = 'multinuc' and parent=? and rst_nodes.doc=? and rst_nodes.project=? and user=?",
        (node_id, doc, project, user),
    )
    return int(count[0][0])
...
def get_multinuc_children_lr(node_id, doc, project, user):
    lr = generic_query(
        "SELECT min(rst_nodes.left), max(rst_nodes.right) FROM rst_nodes JOIN rst_relations ON rst_nodes.relname = rst_relations.relname and rst_nodes.doc = rst_relations.doc and rst_nodes.project = rst_relations.project WHERE reltype = 'multinuc' and parent=? and rst_nodes.doc=? and rst_nodes.project=? and user=?",
        (node_id, doc, project, user),
    )
...
def get_multinuc_children_lr_ids(node_id, left, right, doc, project, user):
    id_left = generic_query(
        "SELECT id FROM rst_nodes JOIN rst_relations ON rst_nodes.relname = rst_relations.relname and rst_nodes.doc = rst_relations.doc and rst_nodes.project = rst_relations.project WHERE reltype = 'multinuc' and parent=? and rst_nodes.left=? and rst_nodes.doc=? and rst_nodes.project=? and user=? ORDER BY rst_nodes.left",
        (node_id, left, doc, project, user),
    )
    id_right = generic_query(
        "SELECT id FROM rst_nodes JOIN rst_relations ON rst_nodes.relname = rst_relations.relname and rst_nodes.doc = rst_relations.doc and rst_nodes.project = rst_relations.project WHERE reltype = 'multinuc' and parent=? and rst_nodes.right=? and rst_nodes.doc=? and rst_nodes.project=? and user=? ORDER BY rst_nodes.left",
...
def count_span_children(node_id, doc, project, user):
    count = generic_query(
        "SELECT count(id) FROM rst_nodes WHERE relname = 'span' and parent=? and rst_nodes.doc=? and rst_nodes.project=? and user=?",
        (node_id, doc, project, user),
    )
    return int(count[0][0])
...
def node_exists(node_id, doc, project, user):
    count = generic_query(
        "SELECT count(*) FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
        (node_id, doc, project, user),
    )
...
def get_rel_type(relname, doc, project):
    if relname == "span" or relname == "":
        ...
        return generic_query(
            "SELECT reltype from rst_relations WHERE relname=? and doc=? and project=?",
            (relname, doc, project),
        )[0][0]
...
def delete_node(node_id, doc, project, user):
    if node_exists(node_id, doc, project, user):
        ...
        if (
            not get_kind(node_id, doc, project, user) == "edu"
        ):  # If it's not an EDU, it may be deleted
            ...
            generic_query(
                "DELETE FROM rst_signals WHERE source=? and doc=? and project=? and user=?",
                (node_id, doc, project, user),
            )
        if not parent == "0":
            if not count_children(parent, doc, project, user) > 0:
                ...
                get_kind(parent, doc, project, user) == "span"
                and count_span_children(parent, doc, project, user) == 0
            ):  # Span just lost its last span child, delete it
                delete_node(parent, doc, project, user)
            elif (
...
def insert_parent(node_id, new_rel, node_kind, doc, project, user):
    ...
    add_node(
        new_parent, lr[0], lr[1], old_parent, old_rel, "", node_kind, doc, project, user
    )
...
def reset_rst_doc(doc, project, user):
    generic_query(
        "DELETE FROM rst_nodes WHERE doc=? and project=? and user=?",
        (doc, project, user),
    )
    generic_query(
        """INSERT INTO rst_nodes (id, left, right, parent, depth, kind, contents, relname, doc, project, user)
	              SELECT id, left, right, parent, depth, kind, contents, relname, doc, project, '"""
        + user
        + "' FROM rst_nodes WHERE doc=? and project=? and user='_orig'"
        "",
        (doc, project),
    )
    generic_query(
        "DELETE FROM rst_signals WHERE doc=? and project=? and user=?",
        (doc, project, user),
    )
    generic_query(
        """INSERT INTO rst_signals (source, type, subtype, tokens, doc, project, user)
	              SELECT source, type, subtype, tokens, doc, project, '"""
        + user
        + "' FROM rst_signals WHERE doc=? and project=? and user='_orig'"
        "",
        (doc, project),
    )
...
def get_children(parent, doc, project, user):
    return generic_query(
        "SELECT id from rst_nodes WHERE parent=? and doc=? and project=? and user=?",
        (parent, doc, project, user),
    )


def get_max_node_id(doc, project, user):
    return generic_query(
        "SELECT max(CAST (id as decimal)) as max_id from rst_nodes WHERE doc=? and project=? and user=?",
        (doc, project, user),
    )[0][0]
...
def get_max_right(doc, project, user):
    return generic_query(
        "SELECT max(right) as max_right from rst_nodes WHERE doc=? and project=? and user=?",
        (doc, project, user),
    )[0][0]
...
def get_users(doc, project):
    return generic_query(
        "SELECT user from rst_nodes WHERE doc=? and project=? and not user='_orig'",
        (doc, project),
    )
...
def get_tok_map(doc, project, user):
    rows = generic_query(
        "SELECT id, contents FROM rst_nodes WHERE kind='edu' and doc=? and project=? and user=? ORDER BY CAST(id AS int)",
        (doc, project, user),
    )
    all_tokens = {}
    token_counter = 0
...
def push_up(push_above_this_seg, doc, project, user):
    ids_above_push = generic_query(
        "SELECT id from rst_nodes WHERE CAST(id as int) > ? and doc=? and project=? and user=? ORDER BY CAST(id as int) DESC",
        (push_above_this_seg, doc, project, user),
    )
    for (
        row
    ) in ids_above_push:  # Do this row-wise to avoid sqlite unique constraint behavior
        id_to_increment = row[0]
        generic_query(
            "UPDATE rst_nodes set id = CAST((CAST(id as int) + 1) as text) WHERE id=? and doc=? and project=? and user=?",
            (id_to_increment, doc, project, user),
        )
        generic_query(
            "UPDATE rst_signals set source = CAST((CAST(source as int) + 1) as text) WHERE source=? and doc=? and project=? and user=?",
            (id_to_increment, doc, project, user),
        )
    generic_query(
        "UPDATE rst_nodes set parent = CAST((CAST(parent as int) + 1) as text) WHERE CAST(parent as int)>? and doc=? and project=? and user=?",
        (push_above_this_seg, doc, project, user),
    )
    generic_query(
        "UPDATE rst_nodes set left = left + 1 WHERE left>? and doc=? and project=? and user=?",
        (push_above_this_seg, doc, project, user),
    )
    generic_query(
        "UPDATE rst_nodes set right = right + 1 WHERE right>? and doc=? and project=? and user=?",
        (push_above_this_seg, doc, project, user),
    )
...
def push_down(push_above_this_seg, doc, project, user):
    ids_above_push = generic_query(
        "SELECT id from rst_nodes WHERE CAST(id as int) > ? and doc=? and project=? and user=? ORDER BY CAST(id as int)",
        (push_above_this_seg, doc, project, user),
    )
    for (
        row
        ...
        generic_query(
            "UPDATE rst_nodes set id = CAST((CAST(id as int) - 1) as text) WHERE id=? and doc=? and project=? and user=?",
            (id_to_decrement, doc, project, user),
        )
        generic_query(
            "UPDATE rst_signals set source = CAST((CAST(source as int) - 1) as text) WHERE source=? and doc=? and project=? and user=?",
            (id_to_decrement, doc, project, user),
...
def update_seg_contents(id, contents, doc, project, user):
    generic_query(
        "UPDATE rst_nodes set contents=? WHERE id=? and doc=? and project=? and user=?",
        (contents, id, doc, project, user),
    )


def get_seg_contents(id, doc, project, user):
    return generic_query(
        "SELECT contents from rst_nodes WHERE id=? and doc=? and project=? and user=?",
        (id, doc, project, user),
    )[0][0]
...
def merge_seg_forward(last_tok_num, doc, project, user):
    ...
    generic_query(
        "DELETE FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
        (str(int(seg_to_merge_forward) + 1), doc, project, user),
    )
    generic_query(
        "DELETE FROM rst_signals WHERE source=? and doc=? and project=? and user=?",
        (str(int(seg_to_merge_forward) + 1), doc, project, user),
...
def copy_doc_to_user(doc, project, user):
    doc_to_copy = generic_query(
        "SELECT id, left, right, parent, depth, kind, contents, relname, doc, project FROM rst_nodes WHERE doc=? and project=? and user='_orig'",
        (doc, project),
    )
    copy = []
    ...
    signals_to_copy = generic_query(
        "SELECT source, type, subtype, tokens, doc, project FROM rst_signals WHERE doc=? and project=? and user='_orig'",
        (doc, project),
    )
    copy = []
...
def get_assigned_users():
    return generic_query(
        "SELECT DISTINCT user from docs where not user = '_orig' ORDER BY user COLLATE NOCASE",
        (),
    )
...
def get_assignments(user):
    return generic_query(
        "SELECT DISTINCT doc, project from docs where user=? ORDER BY user COLLATE NOCASE",
        (str(user),),
    )
...
def delete_doc_user_version(doc, project, user):
    ...
    generic_query(
        "DELETE FROM docs WHERE doc=? and project=? and user=?", (doc, project, user)
    )


def get_node_lr(node_id, doc, project, user):
    left = generic_query(
        "SELECT left FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
        (node_id, doc, project, user),
    )[0][0]
    right = generic_query(
        "SELECT right FROM rst_nodes WHERE id=? and doc=? and project=? and user=?",
        (node_id, doc, project, user),
    )[0][0]
    return [left, right]
    ...
    for action in actions:
        if len(action) > 1:
            generic_query(
                ...
                (doc, project, user, action, mode, time),
            )
...
def get_setting(setting):
    ...
    if schema > 1:
        try:
            return generic_query(
                "SELECT svalue FROM settings where setting=?", (setting,)
            )[0][0]
        except IndexError:
            return ""
    else:
...
def set_guidelines_url(project, guideline_url):
    generic_query(
        "UPDATE projects SET guideline_url=? where project=?", (guideline_url, project)
    )
...
def get_guidelines_url(project):
    ...
    if schema < 2 or project == "":
        ...
        guidelines = generic_query(
            "select guideline_url FROM projects where project=?", (project,)
        )[0][0]
...
doc=? AND project=? AND user=?;"""
    ...
    if len(floating) > 0:
        ...
        del_query = (
            "DELETE from rst_nodes where ID in ("
            + id_string
            + ") and doc=? and project=? and user=?;"
        )
        generic_query(del_query, (doc, project, user))
        del_query = (
            "DELETE from rst_signals where source in ("
            + id_string
            + ") and doc=? and project=? and user=?;"
        )
        generic_query(del_query, (doc, project, user))
...
def update_signals(signals_blob, doc, project, user):
    ...
    generic_query(
        "DELETE from rst_signals WHERE doc=? and project=? and user=?", params
    )
...
def get_signals(doc, project, user):
    ...
    if schema < 6:
        update_schema()
    return generic_query(
        "SELECT source, type, subtype, tokens FROM rst_signals WHERE doc=? and project=? and user=?",
        (doc, project, user),
    )
...
def get_signal_types(doc, project):
    return generic_query(
        "SELECT majtype, subtype FROM rst_signal_types WHERE doc=? and project=?",
        (doc, project),
    )
