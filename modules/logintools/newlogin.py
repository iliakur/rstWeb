...
def donewlogin(theform, userdir, thisscript, action=None):
    ...
    link_url = savedetails(
        userdir, formdict, action
    )  # this includes any 'extra' keys, not just the ones we require
...
def doeditaccount(theform, userconfig, userdir, thisscript, action, newcookie):
    ...
    if pass1 or pass2:
        if pass1 != pass2:
            ...
            display_edit(
                formdict, userdir, thisscript, msg, action, newcookie, userconfig
            )
        if len(pass1) < 5:
        ...
        newcookie = makecookie(
            userconfig, pass1, ConfigObj(userdir + "config.ini")["cookiepath"]
        )
...
def filltemplate(template, formdict=None, vallist=None):
    ...
    if index == -1:
        ...
        keynames = [
            entry.strip()
            for entry in template[start:index].split(",")
...
def validatelogin(formdict, userdir, thisscript, action):
    ...
    if (
        os.path.isfile(userdir + name + ".ini")
        or name in pendinglist
        or name.lower() in RESERVEDNAMES
    ):
...
def validatepass(formdict, userdir, thisscript, action):
    ...
    if pass1 != pass2:
        invalidentry(
            "The two passwords don't match.", formdict, userdir, thisscript, action
        )
    if len(pass1) < 5:
        invalidentry(
...
def invalidentry(msg, formdict, userdir, thisscript, action=None):
    ...
    newloginpage = newloginpage.replace(
        "<!-- **message** -->", "<h2>" + msg + "</h2><br>"
    )
    #
    if action:
        newloginpage = newloginpage.replace("<!-- **action** -->", actionline % action)
...
def newloginfail():
    ...
    error(
        "Sorry, you have either already confirmed your login, "
        "or it is out of date :-)"
    )


def display_edit(
    formdict, userdir, thisscript, msg, action, newcookie, userconfig=None
):
    """
