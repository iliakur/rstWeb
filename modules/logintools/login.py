...
def login(theform, userdir, thisscript=None, action=None):
    ...
    if loginaction.startswith("login"):
        action, userconfig, newcookie = dologin(
            theform, userdir, thisscript, action
        )  # this function displays login and exits if username doesn't exist or password doesn't match
        # if this function returns, then the login attempt was succesful
        userconfig["numlogins"] = str(
            int(userconfig["numlogins"]) + 1
        )  # increment the number of logins in the user config
        userconfig.write()
    ...
    if br:
        ...
        displaylogin(
            userdir, thisscript
        )  # we don't use 'action' in this case because user has explicitly logged out, outstanding action is lost.
    elif loginaction == "newlogin":
        from newlogin import newlogin

        newlogin(
            userdir, thisscript, action
        )  # this script always exits - so no return, preserves action though if needed
        ...
        action, userconfig, newcookie = confirm(
            theform, userdir, thisscript
        )  # no action sent in, because the action is stored in the temporary user store
        userconfig["created"] = str(time())
    elif loginaction.startswith("editaccount"):
        from newlogin import editaccount

        if action[:11] != "edacc-mjf||":
            action, userconfig, newcookie = checklogin(
                userdir, thisscript, "edacc-mjf||" + action
            )
        action = action[11:]
        if not istrue(userconfig["editable"]):
            error("Sorry, this account cannot be edited.")
        editaccount(
            userdir, thisscript, userconfig, sortaction(action), newcookie
        )  # we have checked that we have a valid login. This script prints a page, so no return
        ...
        if not istrue(userconfig["editable"]):
            error("Sorry, this account cannot be edited.")
        action, userconfig, newcookie = doeditaccount(
            theform, userconfig, userdir, thisscript, sortaction(action), newcookie
        )  # we have checked that we have a valid login
    elif loginaction == "admin":
        from admin import admin

        if action[:11] != "admin-mjf||":
            action, userconfig, newcookie = checklogin(
                userdir, thisscript, "admin-mjf||" + action
            )
        action = action[11:]
        adminlevel = int(userconfig["admin"])
        if not adminlevel:
            error("You're not authorised to do that. Sorry.")
        action, userconfig, newcookie = admin(
            theform, userdir, thisscript, userconfig, sortaction(action), newcookie
        )  # we have checked that we have a valid login.

    else:
        displaylogin(
            userdir, thisscript, action
        )  # we haven't understood - just display the login screen XXXX error message instead ?

    print(newcookie)  # XXXX ought to be a way of returning the cookie object instead...
    if (
        action == "EMPTY_VAL_MJF"
    ):  # the programmer ought never to 'see' this value, although it might get passed to his script by the login code a few times....
        action = None
...
def dologin(theform, userdir, thisscript=None, action=None):
    ...
    loginaction = theform[
        "login"
    ].value  # used when we have js and nojs versions of dologin
...
def displaylogin(userdir, thisscript=None, action=None, failed=False):
    ...
    if failed:
        loginpage = loginpage.replace(
            "**login failed**", '<b style="color:red">Wrong user name or password</b>'
        )
    else:
    ...
    if istrue(config["newloginlink"]):
        ...
        loginpage = loginpage.replace(
            "**new login link**", thisscript + "?login=newlogin"
        )
...
def encodestring(username, password):
    ...
    thishash = hashlib.sha1(password + ranstring).hexdigest()
    return pass_enc(
        "||".join([username, thishash, ranstring]), daynumber=True, timestamp=True
    )
