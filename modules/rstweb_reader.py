...
def read_rst(filename, rel_hash, do_tokenize=False):
    ...
    for rel in item_list:
        ...
        if rel.hasAttribute("type"):
            rel_hash[
                relname + "_" + rel.attributes["type"].value[0:1]
            ] = rel.attributes["type"].value
    ...
    all_node_ids = set(
        range(1, id_counter + 1)
    )  # All non-zero IDs in documents, which a signal may refer back to
    ordered_id["0"] = 0
    ...
    for segment in item_list:
        ...
        if relname in schemas:
            ...
            relname = re.sub(
                r"[:;,]", "", relname
            )  # remove characters used for undo logging, not allowed in rel names
    ...
    for signal in item_list:
        ...
        if source not in all_node_ids:
            raise IOError(
                "Invalid source node ID for signal: "
                + str(source)
                + " (from XML file source="
                + signal.attributes["source"].value
                + "(\n"
            )
