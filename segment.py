...
def segment_main(user, admin, mode, **kwargs):
    ...
    edit_bar = edit_bar.replace(
        'id="nav_segment" class="nav_button"',
        'id="nav_segment" class="nav_button nav_button_inset"',
    )
    ...
    help_match = re.search(
        r"(<div.*?help_seg.*?</div>)", help, re.MULTILINE | re.DOTALL
    )
    ...
    for seg_id in segs:
        ...
        if first_seg:
            ...
            cpout += (
                '<div class="tok_space" id="tok'
                + str(tok_counter)
                + '" style="display:none" onclick="act('
                + "'ins:"
                + "tok"
                + str(tok_counter)
                + "'"
            ...
            cpout += (
                '\t\t\t<div id="segend_post_tok'
                + str(tok_counter)
                + '" class="seg_end" onclick="act('
                + "'del:"
                + "tok"
                + str(tok_counter)
                + "'"
    ...
    incorrect_segs = [
        token_key
        for token_key in del_token_to_seg
        if int(tok_seg_map[token_key]) + 1 != del_token_to_seg[token_key]
    ]
    warning = ""
