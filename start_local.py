...
class Root(object):
    ...
    def structure(self, **kwargs):
        ...
        if "current_doc" not in kwargs:
            ...
            cherrypy.response.headers["Content-Disposition"] = (
                'attachment; filename="' + kwargs["current_doc"] + '.png"'
            )
            if sys.version_info[0] == 2:
                ...
                return file_generator(
                    BytesIO(b64decode(structure_main("local", "3", "local", **kwargs)))
                )
        else:
    ...
    def quick_export(self, **kwargs):
        ...
        if "quickexp_doc" not in kwargs:
            ...
            cherrypy.response.headers["Content-Disposition"] = (
                'attachment; filename="' + kwargs["quickexp_doc"] + '"'
            )
...
conf = {
    ...
    "/script": {
        "tools.staticdir.on": True,
        "tools.staticdir.dir": os.path.join(current_dir, "script"),
    },
}
